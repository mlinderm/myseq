...
describe('CoordinateSearchBox', () => {
  ...
  describe('rsId queries', () => {
    ...
    test('Uses MyVariantInfo to obtain hg19/b37 coordiantes for rsIDs', () => {
      const search = shallow(<CoordinateSearchBoxImpl coordinateQuery={jest.fn()} source={source} settings={{ external: true }} error={false} helpMessage="" />);
  ...
  test('Does not make any external requests when prohibited by settings', () => {
    ...
    expect.assertions(1);
    const search = shallow(<CoordinateSearchBoxImpl coordinateQuery={jest.fn()} source={source} settings={{ external: false }} error={false} helpMessage="" />);
