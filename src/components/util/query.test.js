...
const b37Variant = new VCFVariant(
  '9\t22134094\trs10811661\tT\tC\t.\tPASS\t.\tGT\t1/1',
  ['NA12878']
);
const hg38Variant = new VCFVariant(
  'chr9\t22134095\trs10811661\tT\tC\t.\tPASS\t.\tGT\t1/1',
  ['NA12878']
);
const variant = {
  ctg: '9',
...
describe('Reference aware query helper', () => {
  ...
  describe('Reference is b37', () => {
    ...
    test('Invokes variants with correct position', async () => {
      await expect(
        refAwareVariantQuery(mockVCFSource, variant, false)
      ).resolves.toEqual(b37Variant);
      expect(mockVCFSource.variant).toHaveBeenCalledWith(
        '9',
        22134094,
        'T',
        'C',
    ...
    test('Passes assumeRefRef', async () => {
      await expect(
        refAwareVariantQuery(mockVCFSource, variant, true)
      ).resolves.toEqual(b37Variant);
      expect(mockVCFSource.variant).toHaveBeenCalledWith(
        '9',
        22134094,
  ...
  describe('Reference is hg38', () => {
    ...
    test('Rejects if there hg38 coordinates are not supplied', async () => {
      ...
      await expect(
        refAwareVariantQuery(mockVCFSource, variant_noHg38, false)
